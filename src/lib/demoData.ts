...
// Field conventions mirror src/app/api/ext-members/route.ts (the 'api' mode
// conversion) so 'static' mode exercises the same code paths: `type` is set
// explicitly for a4d/associate entries (not left to id-prefix guessing —
// several of these use P/D-prefixed ids that don't match their quota type),
// and `quotaNote` follows the same "4(d) via X" / "Associate via X" text.
...
export const familyMembers: Member[] = [
  ...
  // PO-10 (PT-7-এর beside spouse) → নিজের quota-য় associate A/C
  {
    id: 'AS-151',
    name: 'Ahnaf Hamid Chowdhury',
    via: 'associate',
    type: 'Associate',
    quotaNote: 'Associate via PO-10',
    gender: 'M',
    since: '10/02/2023',
    pid: 'PO-10', rel: 'child',
    fatherId: 'PO-10', motherId: 'PT-7',
  ...
  // successor-এর নিজের quota-য় associate
  {
    id: 'AS-157',
    name: 'Syed Erfan Siddique',
    via: 'associate',
    type: 'Associate',
    quotaNote: 'Associate via LS-35',
    gender: 'M',
    since: '',
    pid: 'LS-35', rel: 'child',
    fatherId: 'LS-35',
