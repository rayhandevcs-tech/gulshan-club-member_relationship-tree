...
export const getRelLabel = (member: Member): string => {
  ...
  if (member.rel === 'child' || member.rel === 'grandchild') {
    const isGrand = member.rel === 'grandchild';
    if (member.via === 'a4d' && member.gender) {
      const base = member.gender === 'M'
        ? (isGrand ? 'Grandson' : 'Son')
        : (isGrand ? 'Granddaughter' : 'Daughter');
      return `${base} (A4D)`;
    }
    return '';
  }
...
export const getRelRoleLabel = (member: Member): string => {
  ...
  if (member.rel === 'grandchild' && member.gender) {
...
const relEdgeLabel = (m: Member): string => {
  ...
  if (m.via === 'succession') return 'Succession';
  switch (m.rel) {
    case 'child': return 'Children';
    case 'grandchild': return 'Grandchildren';
    default: return '';
  }
};
