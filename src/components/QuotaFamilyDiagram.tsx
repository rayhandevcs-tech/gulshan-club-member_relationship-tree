...
// QuotaFamilyDiagram.tsx
// ─────────────────────────────────────────────────────────────────────────────
// Rendering layer only. All graph-building + layout logic lives in
// quotaTreeLayout.ts (pure, unit-testable). See that file for the data model.
//
// Card tiers (exactly two):
//   member card — every independently-standing card: root, children, beside
//                 spouses AND successors (successors keep their amber styling
//                 but share the same size, so ranks stay perfectly aligned)
//   slot card   — every via:'a4d' holder, nested ones included (nested slots
//                 are visually muted instead of resized)
// ─────────────────────────────────────────────────────────────────────────────
...
import {
  ...
  getType, isDead, dispName,
  CARD_W, SLOT_W, CONN_COLOR, TEST_PHOTO_URL
    ...
    <div style={{ position: 'relative' }}>
      ...
      >
        <div style={{
          ...
          //backgroundImage: m.photoUrl ? `url(${m.photoUrl})` : undefined,
          backgroundImage: `url(${m.photoUrl || TEST_PHOTO_URL})`,
          backgroundSize: 'cover', backgroundPosition: 'center',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
